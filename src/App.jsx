...
const RATE_TIERS = [
  { id: "ice",   label: "Standard ICE",      sublabel: "Petrol & diesel cars",       rate: 0.026,  displayRate: "2.60%", color: "#94a3b8", icon: "⛽" },
  { id: "green", label: "Green EV / Hybrid", sublabel: "Electric & hybrid vehicles", rate: 0.0208, displayRate: "2.08%", color: "#4ade80", icon: "🌿" },
...
const FEATURED_CARS = [
  ...
  { id: "model3",  name: "Tesla Model 3",         short: "Tesla Model 3", type: "Electric Sedan",  price: 189888, omv: 32000, coeCategory: "Cat A", loanCap: 60, emoji: "⚡", badge: "Tech Icon",     badgeColor: "#a78bfa", rateTier: "tesla", desc: "Silicon Valley's sedan. Over-the-air updates, Autopilot, zero emissions." },
];
...
function calcAffordability({ salaryRaw, downpaymentRaw, tenure, car }) {
  ...
  const iceInterest = maxLoan * RATE_TIERS.find(t => t.id === "ice").rate * tenure;
  ...
  if (!canAffordDownpayment) {
    verdict = "Insufficient Downpayment"; verdictColor = "#f87171"; verdictBg = "rgba(248,113,113,0.07)"; verdictBorder = "rgba(248,113,113,0.3)";
    ...
    verdict = "Affordable"; verdictColor = "#4ade80"; verdictBg = "rgba(74,222,128,0.07)"; verdictBorder = "rgba(74,222,128,0.3)";
    ...
    verdict = "Stretch"; verdictColor = "#fbbf24"; verdictBg = "rgba(251,191,36,0.07)"; verdictBorder = "rgba(251,191,36,0.35)";
    ...
    verdict = "Out of Range"; verdictColor = "#f87171"; verdictBg = "rgba(248,113,113,0.07)"; verdictBorder = "rgba(248,113,113,0.3)";
    explanation = `Instalment of ${SGD(monthlyInstalment)} is ${(instalmentRatio * 100).toFixed(1)}% of take-home — exceeds the 45% limit.`;
  }
  return { car, tier, maxLoan, requiredDownpayment, downpaymentShortfall, canAffordDownpayment, monthlyInstalment, totalInterest, totalRepayable, takeHome, maxAffordableInstalment, instalmentRatio, months, verdict, verdictColor, verdictBg, verdictBorder, explanation, loanCapPct, downCapPct, annualRate, interestSavings, monthlySavings };
}
...
function useCountUp(target, duration = 900, delay = 0) {
  ...
  useEffect(() => {
    fromRef.current = 0; setDisplay(0); startRef.current = null;
...
// Animated metric card with staggered entrance
function MetricCard({ label, value, sub, accent, highlight, animDelay = 0, visible }) {
  const numVal = parseInt(String(value).replace(/\D/g, ""), 10) || 0;
  const animated = useCountUp(visible ? numVal : 0, 1000, animDelay + 300);
  const prefix = String(value).startsWith("S$") ? "S$" : "";
  const [entered, setEntered] = useState(false);
  useEffect(() => { if (visible) { const t = setTimeout(() => setEntered(true), animDelay); return () => clearTimeout(t); } else { setEntered(false); } }, [visible, animDelay]);
  return (
    <div style={{ background: highlight ? "rgba(167,139,250,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${highlight ? "rgba(167,139,250,0.25)" : accent ? "rgba(217,119,6,0.2)" : "rgba(255,255,255,0.06)"}`, borderRadius: "3px", padding: "14px 16px", position: "relative", overflow: "hidden", opacity: entered ? 1 : 0, transform: entered ? "translateY(0)" : "translateY(12px)", transition: `opacity 0.5s ease, transform 0.5s ease` }}>
...
function GaugeBar({ ratio, visible }) {
  ...
  useEffect(() => { if (visible) { const t = setTimeout(() => setBarWidth(pct), 200); return () => clearTimeout(t); } else setBarWidth(0); }, [visible, pct]);
  return (
    <div style={{ marginBottom: "20px" }}>
      ...
      <div style={{ height: "5px", background: "rgba(255,255,255,0.06)", borderRadius: "3px", overflow: "hidden", position: "relative" }}>
        ...
        <div style={{ height: "100%", width: `${barWidth}%`, background: "linear-gradient(90deg,#4ade80 30%,#fbbf24 45%,#f87171 70%)", borderRadius: "3px", transition: "width 1.2s cubic-bezier(0.16,1,0.3,1)" }} />
      </div>
...
function RateComparisonTable({ car, tenure, maxLoan, visible }) {
  const months = tenure * 12;
  const [entered, setEntered] = useState(false);
  useEffect(() => { if (visible) { const t = setTimeout(() => setEntered(true), 600); return () => clearTimeout(t); } else setEntered(false); }, [visible]);
  return (
    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "3px", overflow: "hidden", marginBottom: "16px", opacity: entered ? 1 : 0, transform: entered ? "translateY(0)" : "translateY(10px)", transition: "opacity 0.6s ease, transform 0.6s ease" }}>
...
// The animated verdict card — the star of the show
function VerdictCard({ results, tenure, downpaymentRaw, visible }) {
  const [phase, setPhase] = useState(0);
  // phase 0 = hidden, 1 = card slides in, 2 = glow pulses, 3 = content reveals, 4 = metrics cascade
  ...
  }, [visible, results?.verdict]);

  if (!results) return null;

  const vc = results.verdictColor;
  const glowing = phase >= 2;
  const contentIn = phase >= 3;
  const metricsIn = phase >= 4;

  // Verdict icon
  const verdictIcon = results.verdict === "Affordable" ? "✦" : results.verdict === "Stretch" ? "◈" : "✕";

  return (
    <div style={{
      marginTop: "20px",
      background: "linear-gradient(145deg,#141414,#0f0f0f)",
      border: `1px solid ${phase >= 1 ? results.verdictBorder : "rgba(255,255,255,0.05)"}`,
      borderRadius: "4px",
      overflow: "hidden",
      position: "relative",
      opacity: phase >= 1 ? 1 : 0,
      transform: phase >= 1 ? "translateY(0) scale(1)" : "translateY(24px) scale(0.98)",
      transition: "opacity 0.5s cubic-bezier(0.16,1,0.3,1), transform 0.5s cubic-bezier(0.16,1,0.3,1), border-color 0.4s ease",
      boxShadow: glowing
        ? `0 0 0 1px rgba(0,0,0,0.8), 0 40px 80px rgba(0,0,0,0.6), 0 0 60px ${vc}18, 0 0 120px ${vc}0a`
        : "0 0 0 1px rgba(0,0,0,0.8), 0 40px 80px rgba(0,0,0,0.6)",
    }}>

      {/* Keyframe styles */}
      <style>{`
        @keyframes topBarGrow { from { transform: scaleX(0); opacity: 0; } to { transform: scaleX(1); opacity: 1; } }
        @keyframes glowPulse { 0%,100% { opacity: 0.6; } 50% { opacity: 1; } }
        @keyframes verdictStamp { 0% { opacity:0; transform: scale(1.4); letter-spacing: 6px; } 60% { opacity:1; transform: scale(0.97); } 100% { transform: scale(1); letter-spacing: -0.5px; } }
        @keyframes iconSpin { from { transform: rotate(-180deg) scale(0); opacity:0; } to { transform: rotate(0deg) scale(1); opacity:1; } }
        @keyframes pctCount { from { opacity:0; transform: translateX(12px); } to { opacity:1; transform: translateX(0); } }
        @keyframes fadeSlideUp { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } }
        @keyframes shimmer { 0% { background-position: -200% center; } 100% { background-position: 200% center; } }
        @keyframes ringPulse { 0% { transform: scale(0.8); opacity:0.8; } 100% { transform: scale(2.5); opacity:0; } }
      `}</style>

      {/* Top colour bar */}
      <div style={{ height: "3px", background: `linear-gradient(90deg,transparent,${vc},transparent)`, transformOrigin: "center", animation: phase >= 1 ? "topBarGrow 0.6s cubic-bezier(0.16,1,0.3,1) forwards" : "none" }} />

      {/* Ambient glow layer */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "200px", background: `radial-gradient(ellipse at 50% 0%, ${vc}12 0%, transparent 70%)`, opacity: glowing ? 1 : 0, transition: "opacity 0.8s ease", pointerEvents: "none" }} />

      {/* ── VERDICT HERO SECTION ── */}
      <div style={{ padding: "32px 40px 28px", borderBottom: "1px solid rgba(255,255,255,0.05)", background: results.verdictBg, position: "relative", overflow: "hidden" }}>

        {/* Ring burst behind icon */}
        {phase >= 2 && (
          <div style={{ position: "absolute", top: "50px", left: "50%", transform: "translateX(-50%)", width: "60px", height: "60px", border: `2px solid ${vc}`, borderRadius: "50%", animation: "ringPulse 0.8s ease-out forwards", pointerEvents: "none" }} />
        )}

        {/* Car + rate pills */}
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "20px", opacity: contentIn ? 1 : 0, transform: contentIn ? "translateY(0)" : "translateY(8px)", transition: "opacity 0.4s ease 0.1s, transform 0.4s ease 0.1s" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "16px" }}>
          ...
          {/* Left: icon + verdict word */}
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            {/* Animated icon circle */}
            <div style={{
              width: "52px", height: "52px", borderRadius: "50%",
              background: `radial-gradient(circle, ${vc}22 0%, ${vc}08 100%)`,
              border: `2px solid ${vc}55`,
              display: "flex", alignItems: "center", justifyContent: "center",
              flexShrink: 0,
              boxShadow: glowing ? `0 0 20px ${vc}40, 0 0 40px ${vc}20` : "none",
              animation: phase >= 2 ? "glowPulse 2s ease-in-out 3" : "none",
              transition: "box-shadow 0.6s ease",
            }}>
              <span style={{
                fontSize: "20px", color: vc,
                display: "inline-block",
                animation: phase >= 2 ? "iconSpin 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards" : "none",
              }}>{verdictIcon}</span>
            </div>

            <div>
              <div style={{ fontFamily: "monospace", fontSize: "9px", letterSpacing: "3px", color: "#57534e", textTransform: "uppercase", marginBottom: "6px", opacity: contentIn ? 1 : 0, transition: "opacity 0.3s ease" }}>Assessment Verdict</div>
              <div style={{
                fontSize: "32px", color: vc, fontStyle: "italic", fontFamily: "Georgia,serif", lineHeight: 1,
                animation: phase >= 3 ? "verdictStamp 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards" : "none",
                opacity: contentIn ? 1 : 0,
                // Shimmer effect on the verdict text
                background: glowing ? `linear-gradient(90deg, ${vc} 0%, #fff 40%, ${vc} 60%, ${vc} 100%)` : vc,
                backgroundSize: "200% auto",
                WebkitBackgroundClip: "text",
                WebkitTextFillColor: "transparent",
                backgroundClip: "text",
                animation: phase >= 3 ? "verdictStamp 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards, shimmer 2.5s linear 1s 2" : "none",
              }}>{results.verdict}</div>
            </div>
          </div>

          {/* Right: percentage */}
          <div style={{ textAlign: "right", animation: contentIn ? "pctCount 0.4s ease 0.3s both" : "none" }}>
            <div style={{ fontFamily: "monospace", fontSize: "9px", letterSpacing: "2px", color: "#57534e", textTransform: "uppercase", marginBottom: "4px" }}>Instalment Burden</div>
            <div style={{ fontFamily: "monospace", fontSize: "36px", color: vc, lineHeight: 1, textShadow: glowing ? `0 0 20px ${vc}80` : "none", transition: "text-shadow 0.6s ease" }}>
              {(results.instalmentRatio * 100).toFixed(1)}<span style={{ fontSize: "16px" }}>%</span>
            </div>
          </div>
        </div>

        {/* Explanation */}
        <p style={{ marginTop: "14px", fontSize: "13px", color: "#a8a29e", lineHeight: 1.7, fontFamily: "Georgia,serif", opacity: contentIn ? 1 : 0, transform: contentIn ? "translateY(0)" : "translateY(6px)", transition: "opacity 0.5s ease 0.3s, transform 0.5s ease 0.3s" }}>
          {results.explanation}
        </p>
      </div>
      ...
      <div style={{ padding: "24px 40px 32px" }}>
        ...
        {/* Gauge */}
        {results.canAffordDownpayment && <GaugeBar ratio={results.instalmentRatio} visible={metricsIn} />}

        {/* Downpayment warning */}
        {!results.canAffordDownpayment && (
          <div style={{ background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: "3px", padding: "14px 16px", marginBottom: "20px", opacity: metricsIn ? 1 : 0, transform: metricsIn ? "translateY(0)" : "translateY(8px)", transition: "opacity 0.5s ease, transform 0.5s ease" }}>
        ...
        {/* Metric cards — staggered cascade */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "16px" }}>
          <MetricCard label="Car Price"          value={`S$${results.car.price}`}                sub={`${results.car.coeCategory} · incl. COE`}             accent   animDelay={0}   visible={metricsIn} />
          <MetricCard label="Max Loan"           value={`S$${Math.round(results.maxLoan)}`}      sub={`${results.car.loanCap}% of purchase price`}                      animDelay={80}  visible={metricsIn} />
          <MetricCard label="Monthly Instalment" value={`S$${Math.round(results.monthlyInstalment)}`} sub={`${tenure} yrs @ ${results.tier.displayRate} flat`}  accent   animDelay={160} visible={metricsIn} />
          <MetricCard label="Total Interest"     value={`S$${Math.round(results.totalInterest)}`} sub={`Total: ${SGD(results.totalRepayable)}`}                          animDelay={240} visible={metricsIn} />
        </div>

        {/* Green savings banner */}
        {results.tier.id !== "ice" && results.interestSavings > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: "12px", background: `${results.tier.color}0f`, border: `1px solid ${results.tier.color}33`, borderRadius: "3px", padding: "12px 16px", marginBottom: "16px", opacity: metricsIn ? 1 : 0, transform: metricsIn ? "translateY(0)" : "translateY(8px)", transition: "opacity 0.5s ease 0.4s, transform 0.5s ease 0.4s" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "13px", color: "#e7e5e4", fontFamily: "Georgia,serif" }}>
                You save <span style={{ color: results.tier.color, fontWeight: "bold" }}>{SGD(results.interestSavings)}</span> total — <span style={{ color: results.tier.color }}>{SGD(results.monthlySavings)}/mo</span> less over {tenure} years.
              </div>
            </div>
          </div>
        )}
        ...
        {/* Rate comparison */}
        <RateComparisonTable car={results.car} tenure={tenure} maxLoan={results.maxLoan} visible={metricsIn} />

        {/* Breakdown table */}
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "3px", overflow: "hidden", marginBottom: results.verdict !== "Affordable" ? "16px" : "0", opacity: metricsIn ? 1 : 0, transition: "opacity 0.5s ease 0.7s" }}>
          {[
            ...
            ["Comfortable max (30%)",     `${SGD(results.maxAffordableInstalment)}/mo`, false],
        ...
        {results.verdict !== "Affordable" && (
          <div style={{ background: "rgba(217,119,6,0.06)", border: "1px solid rgba(217,119,6,0.15)", borderRadius: "3px", padding: "14px 16px", opacity: metricsIn ? 1 : 0, transform: metricsIn ? "translateY(0)" : "translateY(8px)", transition: "opacity 0.5s ease 0.85s, transform 0.5s ease 0.85s" }}>
            ...
            <p style={{ fontSize: "12px", color: "#a8a29e", lineHeight: 1.6, fontFamily: "Georgia,serif", margin: 0 }}>
              {!results.canAffordDownpayment
                ? `Save ${SGD(results.downpaymentShortfall)} more to qualify. The Honda Freed or Toyota Sienta require lower minimums at ~${SGD(163888 * 0.30)}.`
                : results.verdict === "Stretch"
                ? `Extending to 7 years lowers your monthly to ~${SGD(results.maxLoan * (1 + results.annualRate * 7) / 84)}. Or consider a Cat A hybrid for a lower loan amount.`
...
export default function SingaporeCarCalculator() {
  ...
  const handleCalculate = () => {
    ...
    setTimeout(() => {
      ...
      setResults(r);
      setTimeout(() => setShowResults(true), 30);
      ...
      <div style={{ width: "100%", maxWidth: "600px" }}>
        ...
        <div style={{ background: "linear-gradient(145deg,#141414,#0f0f0f)", border: "1px solid rgba(217,119,6,0.25)", borderRadius: "4px", boxShadow: "0 0 0 1px rgba(0,0,0,0.8),0 40px 80px rgba(0,0,0,0.6)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "36px 40px 28px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            ...
            <h1 style={{ fontSize: "28px", fontWeight: "400", color: "#fafaf9", letterSpacing: "-0.5px", lineHeight: 1.2, marginBottom: "8px" }}>Car Affordability<br /><span style={{ color: "#d97706", fontStyle: "italic" }}>Calculator</span></h1>
          ...
          <div style={{ padding: "32px 40px 40px" }}>
            ...
            <div style={{ marginBottom: "28px" }}>
              ...
              {salaryRaw && parseInt(salaryRaw) > 0 && (
                <p style={{ marginTop: "6px", fontSize: "11px", color: "#57534e", fontFamily: "monospace" }}>Take-home after CPF: <span style={{ color: "#d97706" }}>S${Math.floor(parseInt(salaryRaw)*0.8).toLocaleString("en-SG")}/mo</span> · Max instalment: <span style={{ color: "#d97706" }}>S${Math.floor(parseInt(salaryRaw)*0.8*0.3).toLocaleString("en-SG")}/mo</span></p>
              )}
            </div>
        ...
        {/* RESULTS */}
        <div ref={resultsRef}>
          <VerdictCard results={results} tenure={tenure} downpaymentRaw={downpaymentRaw} visible={showResults} />
        </div>
