...
function calcPARF(arf,y) {
  const pcts=[0.75,0.70,0.65,0.60,0.55,0.50];
  const p = y<10 ? (pcts[Math.min(Math.floor(y),5)]||0) : 0;
  return Math.min(arf*p,60000);
}
function estCOE(price,omv){ return Math.max(0,price-omv-calcARF(omv)-omv*0.09-220-price*0.05); }
function calcDepr(car,y){
  const arf=calcARF(car.omv), coe=estCOE(car.price,car.omv);
  const parf=calcPARF(arf,y), coeR=(Math.max(0,120-y*12)/120)*coe;
  const paper=parf+coeR, total=car.price-paper;
  return { arf,coePaid:coe,parf,coeRebate:coeR,paperValue:paper,totalDepr:total,annualDepr:total/y,monthlyDepr:total/y/12 };
}
...
function calc(salary,down,tenure,car){
  ...
  const lcPct=car.loanCap/100, dcPct=1-lcPct;
  const reqDown=car.price*dcPct, loan=car.price*lcPct;
  const canDown=down>=reqDown, months=tenure*12;
  const interest=loan*tier.rate*tenure, repay=loan+interest;
  const monthly=repay/months, takeHome=salary*0.8, maxInst=takeHome*0.3, ratio=monthly/takeHome;
  const saving=loan*0.026*tenure-interest;
  const deprAtTenure=calcDepr(car,tenure);
  const totalCoo=(canDown?down:reqDown)+repay;
  const coo=[1,2,3,5,7].filter(y=>y<=tenure).map(y=>{
    const d=calcDepr(car,y);
    const lp=monthly*y*12, ip=(interest/months)*y*12, pp=lp-ip;
    const du=canDown?down:reqDown;
    return { year:y,done:true,totalPaid:du+lp,downUsed:du,loanPaid:lp,interestPaid:ip,principalPaid:pp,
             netCost:du+lp-d.paperValue,paperValue:d.paperValue,parfNow:d.parf,coeRebateNow:d.coeRebate,
             annualDepr:d.annualDepr,totalDepr:du+lp-d.paperValue };
  });
  // add loan end if not already there
  if(!coo.find(m=>m.year===tenure)){
    const d=calcDepr(car,tenure); const lp=monthly*tenure*12; const ip=interest; const pp=lp-ip; const du=canDown?down:reqDown;
    coo.push({ year:tenure,done:true,totalPaid:du+lp,downUsed:du,loanPaid:lp,interestPaid:ip,principalPaid:pp,
               netCost:du+lp-d.paperValue,paperValue:d.paperValue,parfNow:d.parf,coeRebateNow:d.coeRebate,
               annualDepr:d.annualDepr,totalDepr:du+lp-d.paperValue });
  }
  coo.sort((a,b)=>a.year-b.year);
  const uniqueCoo=[...new Map(coo.map(m=>[m.year,m])).values()];
  ...
  return {car,tier,loan,reqDown,canDown,shortfall:Math.max(0,reqDown-down),
          ...
          verdict,vc,vbg,vborder,lcPct,saving,coo:uniqueCoo,totalCoo,deprAtTenure};
}
...
// ─── SHARED ──────────────────────────────────────────────────────────────────
function Divider({label}){ return <div style={{display:"flex",alignItems:"center",gap:12,margin:"20px 0"}}><div style={{flex:1,height:1,background:T.border}}/><span style={{fontSize:T.xs,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.1em"}}>{label}</span><div style={{flex:1,height:1,background:T.border}}/></div>; }

function MoneyInput({label,hint,value,onChange}){
  const [focused,setFocused]=useState(false); const filled=value!=="";
  return <div><label style={{display:"block",fontSize:T.sm,fontWeight:600,color:T.textSecondary,marginBottom:8}}>{label}</label><div style={{position:"relative"}}><span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:T.sm,fontWeight:600,color:focused||filled?T.gold:T.textMuted,pointerEvents:"none",transition:"color 0.2s"}}>S$</span><input type="text" inputMode="numeric" value={value} onChange={onChange} placeholder="0" onFocus={e=>{setFocused(true);e.target.style.borderColor="rgba(217,119,6,0.6)";e.target.style.boxShadow="0 0 0 3px rgba(217,119,6,0.12)";}} onBlur={e=>{setFocused(false);e.target.style.borderColor=filled?"rgba(217,119,6,0.4)":T.border;e.target.style.boxShadow="none";}} style={{width:"100%",background:"rgba(255,255,255,0.04)",border:`1px solid ${filled?"rgba(217,119,6,0.4)":T.border}`,borderRadius:T.r,padding:"12px 14px 12px 36px",color:T.textPrimary,fontSize:T.lg,fontFamily:"'DM Mono',monospace",fontWeight:500,outline:"none",transition:"border-color 0.2s,box-shadow 0.2s"}}/></div>{hint&&<p style={{marginTop:5,fontSize:T.xs,color:T.textMuted,lineHeight:1.5}}>{hint}</p>}</div>;
}
...
function StatBox({label,value,sub,accent,delay=0,visible}){
  ...
  return <div style={{background:accent?"rgba(217,119,6,0.07)":"rgba(255,255,255,0.03)",border:`1px solid ${accent?"rgba(217,119,6,0.2)":T.border}`,borderRadius:T.r,padding:"14px 16px",opacity:in_?1:0,transform:in_?"translateY(0)":"translateY(8px)",transition:"opacity 0.4s ease,transform 0.4s ease"}}><div style={{fontSize:T.xs,fontWeight:600,color:T.textMuted,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.08em"}}>{label}</div><div style={{fontSize:T.xl,fontFamily:"'DM Mono',monospace",fontWeight:500,color:accent?T.goldLight:T.textPrimary,lineHeight:1,marginBottom:sub?4:0}}>{pre}{anim.toLocaleString("en-SG")}</div>{sub&&<div style={{fontSize:T.xs,color:T.textMuted,lineHeight:1.4}}>{sub}</div>}</div>;
}
...
function GaugeBar({ratio,visible}){
  ...
  return <div style={{marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><span style={{fontSize:T.sm,color:T.textSecondary,fontWeight:500}}>Monthly burden on take-home pay</span><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:T.sm,fontFamily:"'DM Mono',monospace",fontWeight:600,color}}>{pct.toFixed(1)}%</span><span style={{fontSize:T.xs,color,fontWeight:600,background:`${color}20`,border:`1px solid ${color}55`,borderRadius:4,padding:"2px 8px"}}>{label}</span></div></div><div style={{height:8,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden",position:"relative"}}><div style={{position:"absolute",left:"30%",top:0,bottom:0,width:1,background:"rgba(74,222,128,0.35)"}}/><div style={{position:"absolute",left:"45%",top:0,bottom:0,width:1,background:"rgba(251,191,36,0.35)"}}/><div style={{height:"100%",width:`${w}%`,background:"linear-gradient(90deg,#4ade80 0%,#4ade80 30%,#fbbf24 45%,#f87171 70%)",borderRadius:4,transition:"width 1.2s cubic-bezier(0.16,1,0.3,1)"}}/></div><div style={{display:"flex",justifyContent:"space-between",marginTop:5}}><span style={{fontSize:T.xs,color:T.green}}>Safe ≤ 30%</span><span style={{fontSize:T.xs,color:T.amber}}>Stretch ≤ 45%</span><span style={{fontSize:T.xs,color:T.red}}>Too high &gt; 45%</span></div></div>;
}
...
function COOTimeline({coo,totalCoo,tenure,visible}){
  ...
  const max=totalCoo;
  return <div style={{background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden",marginBottom:14,opacity:in_?1:0,transform:in_?"translateY(0)":"translateY(8px)",transition:"opacity 0.5s ease 0.5s,transform 0.5s ease 0.5s"}}><div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`,background:"rgba(255,255,255,0.02)",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:T.xs,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.1em"}}>Cost of Ownership Milestones</span><span style={{fontSize:T.xs,color:T.textMuted}}>Click a year for breakdown</span></div><div style={{padding:"10px 12px",display:"flex",flexDirection:"column",gap:2}}>{coo.map((m,i)=>{const pct=Math.min((m.totalPaid/max)*100,100);const isLast=m.year===tenure;const isOpen=expanded===m.year;return <div key={m.year}><div className="coo-row" onClick={()=>setExpanded(isOpen?null:m.year)} style={{padding:"10px 8px",background:isOpen?"rgba(217,119,6,0.06)":"transparent"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:T.xs,fontFamily:"'DM Mono',monospace",fontWeight:600,color:isLast?T.goldLight:T.textMuted,minWidth:52}}>Year {m.year}</span>{isLast&&<span style={{fontSize:T.xs,fontWeight:600,color:T.gold,background:"rgba(217,119,6,0.15)",border:"1px solid rgba(217,119,6,0.3)",borderRadius:4,padding:"1px 7px"}}>Loan ends</span>}<span style={{fontSize:T.xs,color:isOpen?T.gold:T.textMuted}}>{isOpen?"▲ hide":"▼ details"}</span></div><div style={{textAlign:"right"}}><span style={{fontSize:T.base,fontFamily:"'DM Mono',monospace",fontWeight:600,color:isLast?T.goldLight:T.textPrimary}}>{SGD(m.totalPaid)}</span></div></div><div style={{height:4,background:"rgba(255,255,255,0.05)",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:in_?`${pct}%`:"0%",background:isLast?`linear-gradient(90deg,${T.gold},${T.goldLight})`:"rgba(255,255,255,0.18)",borderRadius:2,transition:`width ${0.7+i*0.1}s cubic-bezier(0.16,1,0.3,1) ${0.5+i*0.08}s`}}/></div></div>{isOpen&&<div style={{animation:"expandDown 0.22s ease forwards",overflow:"hidden",background:"rgba(217,119,6,0.04)",borderTop:"1px solid rgba(217,119,6,0.1)",borderBottom:"1px solid rgba(217,119,6,0.1)",marginBottom:2}}><div style={{padding:"14px 16px"}}><div style={{fontSize:T.xs,fontWeight:700,color:T.gold,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:12}}>Breakdown at Year {m.year}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>{[["Downpayment",SGD(m.downUsed),T.textPrimary],["Loan instalments",SGD(m.loanPaid),T.textPrimary],["↳ Interest paid",SGD(m.interestPaid),T.red],["↳ Principal paid",SGD(m.principalPaid),T.green],["PARF rebate",SGD(m.parfNow),T.green],["COE rebate",SGD(m.coeRebateNow),T.green],["Paper value",SGD(m.paperValue),T.green],["Net true cost",SGD(m.netCost),T.amber]].map(([k,v,c])=><div key={k} style={{background:"rgba(0,0,0,0.2)",borderRadius:4,padding:"8px 10px"}}><div style={{fontSize:T.xs,color:T.textMuted,marginBottom:3,lineHeight:1.3}}>{k}</div><div style={{fontSize:T.sm,fontFamily:"'DM Mono',monospace",fontWeight:600,color:c}}>{v}</div></div>)}</div><p style={{marginTop:10,fontSize:T.xs,color:T.textMuted,lineHeight:1.6}}><strong style={{color:T.textSecondary}}>Net true cost</strong> = total paid minus what you'd recover selling today (PARF + COE rebates).</p></div></div>}</div>; })}</div><div style={{padding:"12px 16px",borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(255,255,255,0.02)"}}><div><div style={{fontSize:T.sm,fontWeight:600,color:T.textSecondary}}>Total cash out over {tenure} years</div><div style={{fontSize:T.xs,color:T.textMuted,marginTop:2}}>Downpayment + all instalments</div></div><span style={{fontSize:T.xl,fontFamily:"'DM Mono',monospace",fontWeight:600,color:T.goldLight}}>{SGD(totalCoo)}</span></div></div>;
}
...
function DepreciationCard({r,tenure,visible}){
  ...
  return <div style={{background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden",marginBottom:14,opacity:in_?1:0,transform:in_?"translateY(0)":"translateY(8px)",transition:"opacity 0.5s ease,transform 0.5s ease"}}><div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:T.xs,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.1em"}}>Depreciation Impact</span><span style={{fontSize:T.xs,fontWeight:600,color:clr,background:`${clr}18`,border:`1px solid ${clr}44`,borderRadius:4,padding:"2px 8px"}}>{lbl} depreciation</span></div><div style={{padding:16}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>{[["Annual loss",SGD(deprAtTenure.annualDepr),"Value lost per year"],["Monthly loss",SGD(deprAtTenure.monthlyDepr),"Hidden cost per month"],["True monthly",SGD(monthly+deprAtTenure.monthlyDepr),"Instalment + depreciation"]].map(([k,v,h])=><div key={k} style={{background:"rgba(0,0,0,0.2)",borderRadius:4,padding:"10px 12px"}}><div style={{fontSize:T.xs,color:T.textMuted,marginBottom:4,lineHeight:1.3}}>{k}</div><div style={{fontSize:T.base,fontFamily:"'DM Mono',monospace",fontWeight:600,color:T.textPrimary,marginBottom:3}}>{v}</div><div style={{fontSize:"10px",color:T.textDim,lineHeight:1.4}}>{h}</div></div>)}</div><div style={{background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`,borderRadius:T.r,padding:"12px 14px",marginBottom:12}}><div style={{fontSize:T.xs,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>What you recover when you sell / scrap</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{[["ARF paid at purchase",SGD(deprAtTenure.arf),T.textSecondary,"Additional Registration Fee based on OMV"],[`PARF rebate at year ${tenure}`,SGD(deprAtTenure.parf),T.green,"Rebate on ARF if deregistered before 10 years, capped S$60k"],[`COE rebate at year ${tenure}`,SGD(deprAtTenure.coeRebate),T.green,"Unused COE months refunded pro-rated"],["Total paper value",SGD(deprAtTenure.paperValue),T.goldLight,"PARF + COE rebate — what you get back"],["Total value lost",SGD(deprAtTenure.totalDepr),T.red,"Price paid minus paper value"]].map(([k,v,c,h])=><div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,paddingBottom:8,borderBottom:"1px solid rgba(255,255,255,0.04)"}}><div><div style={{fontSize:T.sm,color:T.textSecondary,fontWeight:500}}>{k}</div><div style={{fontSize:T.xs,color:T.textMuted,marginTop:1}}>{h}</div></div><span style={{fontSize:T.sm,fontFamily:"'DM Mono',monospace",fontWeight:600,color:c,flexShrink:0}}>{v}</span></div>)}</div></div><p style={{fontSize:T.sm,color:T.textSecondary,lineHeight:1.75}}>On top of your instalment of <strong style={{color:T.textPrimary}}>{SGD(monthly)}</strong>, this car depreciates ~<strong style={{color:clr}}>{SGD(deprAtTenure.monthlyDepr)}/mo</strong> — making the true effective cost <strong style={{color:T.textPrimary}}>{SGD(monthly+deprAtTenure.monthlyDepr)}/mo</strong>.</p></div></div>;
}
...
function RateTable({car,tenure,loan,visible}){
  ...
  return <div style={{background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden",marginBottom:14,opacity:in_?1:0,transform:in_?"translateY(0)":"translateY(8px)",transition:"opacity 0.5s ease,transform 0.5s ease"}}><div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:T.xs,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.1em"}}>Interest Rate Comparison</span></div>{RATE_TIERS.map((tier,i)=>{ const int_=loan*tier.rate*tenure; const mo=(loan+int_)/months; const active=tier.id===car.rateTier; const disabled=tier.id==="tesla"&&car.rateTier!=="tesla"; return <div key={tier.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",borderBottom:i<2?`1px solid ${T.border}`:"none",background:active?"rgba(217,119,6,0.05)":"transparent",opacity:disabled?0.3:1}}><span style={{fontSize:16}}>{tier.icon}</span><div style={{flex:1}}><div style={{fontSize:T.sm,fontWeight:600,color:active?T.goldLight:T.textSecondary}}>{tier.label}</div><div style={{fontSize:T.xs,color:T.textMuted,marginTop:2}}>{tier.display} flat{disabled?" · Tesla only":""}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:T.base,fontFamily:"'DM Mono',monospace",fontWeight:600,color:active?T.goldLight:T.textSecondary}}>{SGD(mo)}/mo</div><div style={{fontSize:T.xs,color:T.textMuted,marginTop:2}}>Interest: {SGD(int_)}</div></div>{active&&<div style={{width:6,height:6,borderRadius:"50%",background:T.gold,boxShadow:`0 0 6px ${T.gold}`,flexShrink:0}}/>}</div>; })}</div>;
}
  ...
  return (
    <div ref={wrapRef}>
      ...
      <div style={{ marginBottom:14 }}>
        <div style={{ fontSize:T.xs, fontWeight:600, color:T.textMuted, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:8 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
          {TOP5.map(car => {
            ...
            return (
              <button key={car.id} onClick={() => select(car)} className={`top5-btn${sel?" sel":""}`}
        ...
        <div style={{ position:"relative" }} ref={dropRef}>
          ...
          <div style={{ position:"relative" }}>
            ...
            <input
              ...
              style={{ width:"100%", background:"rgba(255,255,255,0.04)", border:`1px solid ${focused?"rgba(217,119,6,0.5)":T.border}`, borderRadius:T.r, padding:"11px 14px 11px 38px", color:T.textPrimary, fontSize:T.sm, outline:"none", transition:"border-color 0.2s, box-shadow 0.2s", boxShadow:focused?"0 0 0 3px rgba(217,119,6,0.1)":"none" }}
            />
            {query && (
              <button onClick={() => { setQuery(""); inputRef.current?.focus(); }} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:T.textMuted, cursor:"pointer", fontSize:16, lineHeight:1, padding:0 }}>✕</button>
            )}
          </div>

          {/* Dropdown results */}
          {showDrop && (
            <div className="zz-scroll" style={{ position:"absolute", top:"calc(100% + 4px)", left:0, right:0, zIndex:999, background:"#161410", border:`1px solid rgba(217,119,6,0.22)`, borderRadius:T.r, boxShadow:"0 16px 48px rgba(0,0,0,0.85)", overflow:"hidden", animation:"expandDown 0.18s ease forwards" }}>
              {filtered.length === 0 ? (
                <div style={{ padding:"16px 14px", fontSize:T.sm, color:T.textMuted, textAlign:"center" }}>
                ...
                const hi  = hiIdx === i;
                return (
                  <button key={car.id} className={`search-row${hi?" active":""}`}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      {/* Highlight match */}
                      <div style={{ fontSize:T.sm, fontWeight:500, color:sel?T.textPrimary:T.textSecondary }}>
                        {highlightMatch(car.name, query)}
                      </div>
                      <div style={{ fontSize:T.xs, color:T.textMuted, marginTop:2 }}>
                        {car.type} · {car.coe} · {car.loanCap}% loan cap
                        {car.top5 ? <span style={{ marginLeft:6, color:T.gold, fontWeight:600 }}>#{car.rank} best-seller</span> : null}
                      </div>
                    </div>
              ...
              {filtered.length > 0 && (
                <div style={{ padding:"8px 14px", borderTop:`1px solid ${T.border}`, fontSize:T.xs, color:T.textDim }}>
      ...
      {value && (
        <div style={{ marginTop:12, padding:"12px 14px", background:T.goldFaint, border:`1px solid rgba(217,119,6,0.18)`, borderRadius:T.r, animation:"fadeUp 0.25s ease forwards" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
            ...
            {value.top5 && (
              <span style={{ marginLeft:"auto", fontSize:T.xs, fontWeight:700, color:T.gold, background:"rgba(217,119,6,0.15)", border:"1px solid rgba(217,119,6,0.3)", borderRadius:4, padding:"2px 8px" }}>#{value.rank} in SG</span>
            )}
          </div>
...
function highlightMatch(text, query) {
  ...
  return (
    <span>
      {text.slice(0, idx)}
      <mark style={{ background:"rgba(217,119,6,0.3)", color:T.goldLight, borderRadius:2, padding:"0 1px" }}>{text.slice(idx, idx+query.length)}</mark>
...
function ResultPanel({r,tenure,visible,slim=false}){
  ...
  if(!r) return <div style={{flex:1,background:"rgba(255,255,255,0.01)",border:`2px dashed ${T.border}`,borderRadius:T.rL,display:"flex",alignItems:"center",justifyContent:"center",minHeight:220}}><div style={{textAlign:"center"}}><div style={{fontSize:28,marginBottom:10,opacity:0.15}}>◻</div><div style={{fontSize:T.sm,color:T.textDim,fontWeight:500}}>Select a car to see results</div></div></div>;
  ...
  return <div style={{flex:1,background:T.surface,border:`1px solid ${phase>=1?vborder:T.border}`,borderRadius:T.rL,overflow:"hidden",opacity:phase>=1?1:0,transform:phase>=1?"translateY(0) scale(1)":"translateY(20px) scale(0.98)",transition:"opacity 0.45s cubic-bezier(0.16,1,0.3,1),transform 0.45s cubic-bezier(0.16,1,0.3,1),border-color 0.4s,box-shadow 0.6s",boxShadow:glowing?`0 0 0 1px rgba(0,0,0,0.8),0 24px 60px rgba(0,0,0,0.5),0 0 50px ${vc}10`:"0 0 0 1px rgba(0,0,0,0.8),0 24px 60px rgba(0,0,0,0.5)"}}>
    ...
    <div style={{padding:slim?"18px 20px 16px":"24px 26px 20px",borderBottom:`1px solid ${T.border}`,background:vbg,position:"relative",overflow:"hidden"}}>
      ...
      <p style={{marginTop:14,fontSize:T.sm,color:T.textSecondary,lineHeight:1.75,opacity:contentIn?1:0,transition:"opacity 0.5s 0.35s"}}>
        {!canDown?<>Min. downpayment is <strong style={{color:T.goldLight}}>{SGD(reqDown)}</strong> ({100-car.loanCap}% of price). You're short by <strong style={{color:T.red}}>{SGD(shortfall)}</strong>.</>
        :verdict==="Affordable"?<>{SGD(monthly)}/mo is {(ratio*100).toFixed(1)}% of take-home — comfortably within the 30% safe threshold. You're in good shape.</>
        :verdict==="Stretch"?<>{SGD(monthly)}/mo is {(ratio*100).toFixed(1)}% of take-home. Manageable, but limited savings buffer each month.</>
    ...
    <div style={{padding:slim?"14px 20px 22px":"18px 26px 28px"}}>
      ...
      {!canDown&&<div style={{background:T.redFaint,border:"1px solid rgba(248,113,113,0.25)",borderRadius:T.r,padding:"14px 16px",marginBottom:16,opacity:metricsIn?1:0,transition:"opacity 0.5s"}}><div style={{fontSize:T.xs,fontWeight:700,color:T.red,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>⚠ Downpayment Shortfall</div><p style={{fontSize:T.sm,color:T.textSecondary,lineHeight:1.65}}>You need <strong style={{color:T.goldLight}}>{SGD(reqDown)}</strong> minimum. Save <strong style={{color:T.red}}>{SGD(shortfall)}</strong> more to qualify.</p></div>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
        ...
        <StatBox label="Maximum loan"        value={`S$${Math.round(loan)}`}     sub={`${car.loanCap}% of price`}              delay={80}  visible={metricsIn}/>
      ...
      {tier.id!=="ice"&&saving>0&&<div style={{display:"flex",alignItems:"flex-start",gap:12,background:`${tier.color}0d`,border:`1px solid ${tier.color}30`,borderRadius:T.r,padding:"12px 16px",marginBottom:14,opacity:metricsIn?1:0,transition:"opacity 0.5s 0.45s"}}><span style={{fontSize:20,flexShrink:0}}>💰</span><div><div style={{fontSize:T.xs,fontWeight:700,color:tier.color,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>Green rate savings vs ICE</div><p style={{fontSize:T.sm,color:T.textSecondary,lineHeight:1.65}}>Save <strong style={{color:tier.color}}>{SGD(saving)}</strong> total — <strong style={{color:tier.color}}>{SGD(saving/(tenure*12))}/mo</strong> less than a petrol loan.</p></div></div>}
      {!slim&&<RateTable car={car} tenure={tenure} loan={loan} visible={metricsIn}/>}
      {verdict!=="Affordable"&&<div style={{background:T.goldFaint,border:`1px solid rgba(217,119,6,0.18)`,borderRadius:T.r,padding:"13px 16px",opacity:metricsIn?1:0,transition:"opacity 0.5s 0.9s"}}><div style={{fontSize:T.xs,fontWeight:700,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>💡 What you could do</div><p style={{fontSize:T.sm,color:T.textSecondary,lineHeight:1.75}}>{!canDown?<>Save <strong style={{color:T.goldLight}}>{SGD(shortfall)}</strong> more. The Honda Freed or Toyota Sienta have lower minimums (~{SGD(163888*0.30)}).</>:verdict==="Stretch"?<>Extending to 7 years lowers monthly to ~<strong style={{color:T.goldLight}}>{SGD(loan*(1+(r?.rate||0.026)*7)/84)}</strong>. A Cat A hybrid also reduces loan size.</>:<>Your comfortable budget is ~<strong style={{color:T.goldLight}}>{SGD(takeHome*0.30*tenure*12/(1+(r?.rate||0.026)*tenure)/lcPct)}</strong>. Consider the Corolla Altis, Sienta, or BYD Dolphin.</>}</p></div>}
    </div>
      ...
      <div className="zz-root" style={{minHeight:"100vh",background:T.bg,backgroundImage:`radial-gradient(ellipse at 20% 50%,rgba(217,119,6,0.05) 0%,transparent 50%),radial-gradient(ellipse at 80% 10%,rgba(180,83,9,0.04) 0%,transparent 40%)`,display:"flex",justifyContent:"center",padding:"40px 16px 80px"}}>
        <div style={{width:"100%",maxWidth:mode==="compare"?1120:640,transition:"max-width 0.4s ease"}}>
          ...
          <div style={{background:T.surface,border:`1px solid rgba(217,119,6,0.2)`,borderRadius:T.rL,boxShadow:"0 0 0 1px rgba(0,0,0,0.6),0 32px 80px rgba(0,0,0,0.4)",overflow:"hidden",marginBottom:16}}>
            ...
            <div style={{padding:"22px 30px 26px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:4}}>
                <MoneyInput label="Monthly Gross Salary" value={salary} onChange={handleSalary} hint={salaryRaw&&parseInt(salaryRaw)>0?`Take-home after CPF: S$${Math.floor(parseInt(salaryRaw)*0.8).toLocaleString("en-SG")}/mo  ·  30% limit: S$${Math.floor(parseInt(salaryRaw)*0.8*0.3).toLocaleString("en-SG")}/mo`:"Gross income before CPF deductions"}/>
                <MoneyInput label="Available Cash Downpayment" value={down} onChange={handleDown} hint="Full cash only — CPF cannot be used for car loans"/>
              </div>
              ...
              <div style={{marginBottom:26}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,padding:"0 10px"}}>
                  {[1,2,3,4,5,6,7].map(y=><div key={y} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}} onClick={()=>setTenure(y)}><div style={{width:1,height:6,background:tenure===y?T.gold:T.border}}/><span style={{fontSize:T.xs,fontFamily:"'DM Mono',monospace",color:tenure===y?T.gold:T.textMuted,fontWeight:tenure===y?600:400}}>{y}</span></div>)}
                </div>
                <input type="range" className="zz-slider" min={1} max={7} step={1} value={tenure} onChange={e=>setTenure(Number(e.target.value))} style={{background:"transparent"}} ref={el=>{if(el){const s=document.getElementById("zz-track-style")||document.createElement("style");s.id="zz-track-style";s.textContent=`.zz-slider::-webkit-slider-runnable-track{background:${trackBg}}`;if(!s.parentNode)document.head.appendChild(s);}}}/>
              ...
              <p style={{marginTop:6,textAlign:"center",fontSize:T.xs,color:T.textDim}}>Prices indicative incl. COE · Based on MAS regulations · For reference only</p>
            </div>
            <div style={{height:1,background:`linear-gradient(90deg,transparent,rgba(217,119,6,0.15),transparent)`}}/>
          </div>
          ...
          <div ref={resultsRef}>
            ...
            <div>
              {(rA||rB)&&<div style={{marginBottom:12,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}><span style={{fontSize:T.sm,fontWeight:600,color:T.textSecondary}}>Side-by-side comparison</span>{rA&&rB&&rA.monthly&&rB.monthly&&<div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderRadius:20,padding:"6px 14px"}}><span style={{fontSize:T.xs,color:T.textMuted}}>Monthly diff:</span><span style={{fontSize:T.sm,fontFamily:"'DM Mono',monospace",fontWeight:700,color:T.amber}}>{SGD(Math.abs(rA.monthly-rB.monthly))}/mo</span><span style={{fontSize:T.xs,color:T.green,fontWeight:500}}>({rA.monthly<rB.monthly?`${rA.car.short} cheaper`:`${rB.car.short} cheaper`})</span></div>}</div>}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,alignItems:"start"}}>
                <ResultPanel r={rA} tenure={dTenure} visible={!!rA} slim/>
                <ResultPanel r={rB} tenure={dTenure} visible={!!rB} slim/>
              </div>
              {rA&&rB&&rA.canDown&&rB.canDown&&<div style={{marginTop:12,background:T.surface,border:`1px solid ${T.border}`,borderRadius:T.rL,padding:"20px 24px"}}><div style={{fontSize:T.base,fontWeight:600,color:T.textPrimary,marginBottom:4}}>Total cost comparison over {dTenure} years</div><p style={{fontSize:T.sm,color:T.textMuted,marginBottom:18}}>Downpayment + all monthly instalments</p>{[{r:rA,slot:"A",color:T.gold},{r:rB,slot:"B",color:"#22d3ee"}].map(({r,slot,color})=>{const maxTco=Math.max(rA.totalCoo,rB.totalCoo);const pct=(r.totalCoo/maxTco)*100;const cheaper=r.totalCoo<=Math.min(rA.totalCoo,rB.totalCoo);return <div key={slot} style={{marginBottom:16}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:7,alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{background:color,color:"#000",fontSize:T.xs,fontWeight:700,borderRadius:3,padding:"2px 7px",fontFamily:"'DM Mono',monospace"}}>{slot}</span><span style={{fontSize:T.sm,fontWeight:500,color:T.textSecondary}}>{r.car.name}</span>{cheaper&&<span style={{fontSize:T.xs,fontWeight:600,color:T.green,background:"rgba(74,222,128,0.1)",border:"1px solid rgba(74,222,128,0.25)",borderRadius:4,padding:"1px 7px"}}>CHEAPER</span>}</div><span style={{fontSize:T.lg,fontFamily:"'DM Mono',monospace",fontWeight:600,color}}>{SGD(r.totalCoo)}</span></div><div style={{height:8,background:"rgba(255,255,255,0.05)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${color}88,${color})`,borderRadius:4,transition:"width 1s cubic-bezier(0.16,1,0.3,1) 0.3s"}}/></div></div>;})}<div style={{marginTop:16,paddingTop:16,borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:T.sm,fontWeight:500,color:T.textSecondary}}>You save choosing {rA.totalCoo<=rB.totalCoo?rA.car.short:rB.car.short}</div><div style={{fontSize:T.xs,color:T.textMuted,marginTop:2}}>over the full {dTenure}-year period</div></div><span style={{fontSize:T.xxl,fontFamily:"'DM Mono',monospace",fontWeight:600,color:T.green}}>{SGD(Math.abs(rA.totalCoo-rB.totalCoo))}</span></div></div>}
            </div>}
          </div>
        </div>
      </div>
